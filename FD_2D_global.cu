...
void updateCpu(float *u, float *u_prev, int N, float h, float dt, float alpha, int BSZ) {
	for (int i = 3; i < N-3; i++) {
		for (int j = 3; j < N-3; j++) {
			// std::cout << u[i*N+j] << std::endl;
			// u[i*N+j] = u_prev[i*N+j] + alpha*dt/(h*h) * (u_prev[i*N+j+1] + u_prev[i*N+j-1] + u_prev[i*N+j+N] + u_prev[i*N+j-N] - 4*u_prev[i*N+j]);
			u[i*N+j] = u[i*N+j] + alpha*dt/(h*h) * (u[i*N+j+1] + u[i*N+j-1] + u[i*N+j+N] + u[i*N+j-N] - 4*u[i*N+j]);
		}
	}
}
...
///
	float *u_  	= new float[N*N];
	float *u_prev_  	= new float[N*N];
...
////
	...
	std::cout<<"time GPU = "<<elapsed<<std::endl;
	...
	for (int t=0; t<steps; t++)
	{
		updateCpu(u_, u_prev_d, N, h, dt, alpha, BLOCKSIZE);
	}
	stop = get_time();
	elapsed = stop - start;
	std::cout<<"time CPU = "<<elapsed<<std::endl;
	/// end CPU
